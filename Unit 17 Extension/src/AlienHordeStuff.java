...
public class AlienHordeStuff
{
	private List<Alien1> aliens;

	public AlienHordeStuff()
	{
		...
		for (int i=0; i<aliens.size(); i++) {
			...
			for (int j=0; j<shots.size(); j++) {
				if (shots.get(j).getX()<aliens.get(i).getX()+aliens.get(i).getWidth()&&shots.get(j).getX()+shots.get(j).getWidth()>aliens.get(i).getX()
					...
					if (shots.get(j).getId()!=20) {
						aliens.get(i).dec(shots.get(j).getD());
						shots.remove(j);
					}
					else {
						shots.get(j).setHeight(shots.get(j).getHeight()-2*Math.min(minZero(aliens.get(i).getLf()), shots.get(j).getD()));
						aliens.get(i).dec(shots.get(j).getD());
					}
				}
			}
